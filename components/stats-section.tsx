...
export default function StatsSection() {
  ...
  const stats = [
    { number: "20", label: "Active Members", description: "Students actively building (since launch)", delay: 0 },
    { number: "4", label: "Projects Completed", description: "Production-ready solutions (since launch)", delay: 0.1 },
    { number: "5", label: "Contributors", description: "Open-source contributors (since launch)", delay: 0.2 },
    { number: "1", label: "Community Partners", description: "Industry collaborations (since launch)", delay: 0.3 },
  ]
    ...
    <section className="py-12 sm:py-16 lg:py-20 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          ...
          <p className="text-xs text-muted-foreground max-w-3xl mx-auto mt-3">
            Transparent early-stage metrics shown since our launch — we're actively growing through new projects, contributors, and partner work.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 lg:gap-8">
          {(() => {
            ...
            return stats.map((stat, index) => (
              <div
                key={index}
                className={`group relative p-4 sm:p-6 lg:p-8 bg-secondary/20 backdrop-blur-sm border border-border/50 rounded-2xl hover:border-primary/30 hover:bg-secondary/30 transition-all duration-500 text-center hover:scale-105 hover:shadow-xl hover:shadow-primary/5 ${isLoaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"} ${delayClasses[index]}`}
              >
